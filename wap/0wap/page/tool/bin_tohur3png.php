...
?>
你正在用文件“<%=$title%>”（<%=fat32_f::echosize($fsize), '，大文件补卷专用通道'%>）生成HuR3png分卷。<?php /*如果你第一次使用它，请先[read=,user,ssh&amp;u=<%=urlencode($_SERVER<('REQUEST_URI')>)%>]登记相册信息[/read]*/ ?>[hr]
[form=post,read.php?[u.b]&amp;[u.c]&amp;[u.p]&amp;dir=<%=$dir%>&amp;eid=<%=$eid%>]
图片描述:[input=content]<%=code::html($title)%>[/input][br]
分卷大小:[input=packsize]0[/input]KB(0为自适应，生成1024KB的分卷)[br]
...
$tp=TEMP_DIR.'/rc/139photo.png';
imgpit::图片字符串($content=$_POST['content'],$tp);
$tpsize=filesize($tp);
if(!$packsize)
 $packsize=1024*1024-$tpsize-32;

$tpnr=file_get_contents($tp);
$tpnr2=substr($tpnr,-12,12);
$tpnr=substr($tpnr,0,-12);
...
//$fp2=fopen($目标目录.'/hur3.tmp.gz','w');
$seek=($i-1)*$packsize;
fwrite($fp,/*dechex($pksize)."\r\n".*/$tpnr.pack('NV',$packsize+28,$fz).($pngtmp='tEXT'.$md5.pack('v2',$packcount,$i)));
//fwrite($fp2,$pngtmp);
fseek($fp0,$seek);
...
fwrite($fp,$pngtmp2=pack('V',$packsize));
$pngtmp.=$pngtmp2;

fwrite($fp,pack('N',crc32($pngtmp)).$tpnr2."\r\n\r\n");
fclose($fp);
//fwrite($fp2,$pngtmp);
//fclose($fp2);
$s['h'.$i]=array('title'=>'h'.$i,'type'=>'png');
if(is_file($breakfile))
 {
  unlink($breakfile);
  break;
