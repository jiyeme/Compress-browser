...
?>
你正在用文件“<%=$title%>”（<%=fat32_f::echosize($fsize), '，大文件补卷专用通道'%>）生成HuR3分卷。<?php /*如果你第一次使用它，请先[read=,user,ssh&amp;u=<%=urlencode($_SERVER<('REQUEST_URI')>)%>]登记相册信息[/read]*/ ?>[hr]
...
$seek=($i-1)*$packsize;
fwrite($fp,$tpnr.pack('V',$fz).$md5.pack('v2',$packcount,$i));
fseek($fp0,$seek);
...
fwrite($fp,pack('V',$packsize).md5_file($目标目录.'/hur3.tmp.gz',true).'HuR3');
fclose($fp);
