...
$nr=strtoupper(bin2hex(fread($fp,5)));
fclose($fp);
echo '文件的前5个字节：[br][url=#]';
for($i=0;$i<10;$i+=2)
{
echo substr($nr,$i,2),' ';
}
...
文件名:[input=fname]<%=str::word(str::pinyin($title))%>.png[/input][br]
...
$gour=substr($lur,26);$lur=explode(';jsessionid=',$lur);
$lur=explode('?',$lur[1]);
$sk['jsessionid']=$lur[0];
...
$tp=TEMP_DIR.'/rc/139photo2.png';
imgpit::单像素图片($tp);
$tplen=filesize($tp);*/
$h->file('upfile',$_POST['fname'],/*file_get_contents($tp)*/pack('H*','89504e470d').substr($nr=file_get_contents($fn),5)/*.pack('L',$fz).$md5.'HuR1'*/,'image/x-png');
...
echo strtoupper(bin2hex(substr($nr,0,5)));
/*$tplen=$tplen*2;
for($i=0;$i<$tplen;$i+=2)
{
echo substr($hex,$i,2);
