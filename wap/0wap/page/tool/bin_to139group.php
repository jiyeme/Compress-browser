...
$breakfile=USERFILE_DIR.'/'.$USER['uid'].'/bin_to139group_break.lock';
if($_GET['break'])
{ 
...
?>[hr]请等待这个任务完成，或者[read=,tool,bin_to139group&amp;break=true]取消任务[/read]（任务不属于你时慎用，后果自负）。[br]<?php
if($isonly)
 echo '（这是你自己的任务）';
else
 echo '（小于5MB的文件不需要等待）';
...
$input=new session($USER['uid'],'input',3600*24*5,array('139group_url'));
if(!$e=$s[$eid])
{ ?>
[html=出错啦！]
...
if(!$_POST['go'])
{ ?>
[html=<%=$title=code::html($e<('title')>)%>-转到139圈子相册]
[head]
<?php
...
?>
你正在把文件“<%=$title%>”（<%=fat32_f::echosize($fsize), $urf ? '，独享通道' : '，大文件共享通道，请抓紧时间发送，否则别人就抢先了'%>）分卷发布到你的139圈子相册。你需要粘贴你的相片上传页地址。<?php /*如果你第一次使用它，请先[read=,user,ssh&amp;u=<%=urlencode($_SERVER<('REQUEST_URI')>)%>]登记相册信息[/read]*/ ?>[hr]
[form=post,read.php?[u.b]&amp;[u.c]&amp;[u.p]&amp;dir=<%=$dir%>&amp;eid=<%=$eid%>]圈子URL:[input=url]<%=code::html($input['139group_url'])%>[/input][br]
...
$input['139group_url']=$lur=$_REQUEST['url'];
$gour=substr($lur,26);
...
$url=/*"http://wap.139.10086.cn:84*/substr($_REQUEST['url'],0,26)."/group.do;jsessionid={$sk['jsessionid']}?1=&m={$sk['m']}&src={$sk['src']}&do=Photo.addPost&gid={$sk['gid']}&go=".urlencode($gour);
$h=new httplib;
$h->open($url,10,5);
...
unlink($lockfile);
echo '[url=http://wap.139.10086.cn:84/group.do;jsessionid='.$sk<('jsessionid')>.'?1=&amp;m='.$sk<('m')>.'&amp;src='.$sk<('src')>.'&amp;do=Photo.findPost&amp;gid=',$sk<('gid')>,']自己去圈子相册看看吧[/url]';
}
?>
