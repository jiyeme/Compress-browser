...
/*
		...
		$mail->AddAttachment($filename,$dir['title'],get_file_mime(get_short_file_mime($dir['title'])));*/
		
		
$mail = new PHPMailer(); //创建实例
$mail -> CharSet='utf-8'; //设置字符集
$mail -> SetLanguage('ch',ROOT_DIR.'PHPMailer/language/');  //设置语言类型和语言文件所在目录          
...
$mail -> AddAttachment($filename,$dir['title']);//附件的路径和附件名称
/*if(!$mail -> Send()) //发送邮件  
...
*/
	...
	if ( $browser->template == 1 ){
		echo '
		邮件地址：<input name="mail'.$browser->rand.'" type="text" value="'.@cloud_memcache::get($browser->uid . '_mail').'"/><br/>
		...
		邮件地址：<input name="mail" type="text" value="'.@cloud_memcache::get($browser->uid . '_mail').'"/><br/>
		(如果文件过大,手机浏览器可能会出现页面超时现象,无视即可,请随时到您的邮箱看看有没有收到邮件.)<br/>
