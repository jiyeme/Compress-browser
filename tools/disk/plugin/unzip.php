...
if ( in_array($b_set['server_method'],array('sae','ace')) ){
	echo '错误:[' . $b_set['server_method'] .']环境下暂时不支持此操作。' ;
...
//压缩包文件
$_file = cloud_storage::localname('tmp_' . rand(0,999) . time());
$_file = cloud_storage::download_tmp('disk_' . $dir['file'],$_file);
...
if ( !is_dir($_dir) ){
	...
	if ( $dir['mime'] == 'mrp' ){
		...
		require ROOT_DIR.'inc/class/pclzip.lib.php';
...
if ( !$is_dir && !$is_file ) {
	...
	require ROOT_DIR.'tools/disk/plugin/unzip_list.php';
}elseif ( $is_file ){
	require ROOT_DIR.'tools/disk/plugin/unzip_info.php';
}else{
