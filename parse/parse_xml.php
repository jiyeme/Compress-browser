...
if ( stripos($html,'<script')!==false && stripos($html,'</script>')!==false  ){
	...
	$html = preg_replace_callback('/<script(.*?)<\/script>/i',function($r){return script_check_jump($r[1]);}, $html);
}
...
if ( stripos($html,'<style')!==false  && stripos($html,'style>')!==false  ){
	$html =preg_replace_callback('/<style(.*?)style>/i',function($r){return '<style'.fix_css($r[1]).'style>';}, $html);
}
...
$html = preg_replace_callback('/<([!a-zA-Z]{1,9}[1-5]{0,1}) (.*?)>/i', function($r){return check_xml($r[1],$r[2]);}, $html);

$browser->cacheurl_set();

//$html = preg_replace('@<([/a-zA-Z1-5]{1,9}[1-5]{0,1})>@ies', "check_xml('\\1','\\2')", $html);
//$html = preg_replace_callback('/<([/a-zA-Z1-5]{1,9}[1-5]{0,1})>/i', function($r){return check_xml($r[1],$r[2]);}, $html);
