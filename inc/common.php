...
!defined('DIR') && define('DIR',"/home/jysafec1/public_html/yl/".'/');
...
Class browser{
   ...
   function site_lists($all=true){
      $site = array();
      $site[] = array('title' => '祭夜Bug','url' => 'http://www.jysafe.cn');
   ...
   private function _set_default(){
      $var = array(
            ...
            'config_pic_wap'   =>   '0',
            'template_foot'      =>   '[main]|[book]|[menu][br][go][br][size][br][time=Y-n-j H:i:s D]',
      );
      return $var;
   }
   ...
   function user_login($name = false, $pass = false ,$template = 0){
      ...
      if ( $var === false ){
         ...
         if ( $type === 0 ){
            //为支持免流做的更改
            $GLOBALS['JIUWAP_SID']=$this->template.';'.$name.';'.$pass;
            //END为支持免流做的更改
            Setcookie('FREE', $this->template.';'.$name.';'.$pass,time_()+2592000);
         }
         return true;
